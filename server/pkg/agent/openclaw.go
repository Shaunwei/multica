...
//     emits today, typically pretty-printed across many lines; gateway mode may
//     wrap it as {runId,status,result}
...
func (b *openclawBackend) processOutput(r io.Reader, ch chan<- Message) openclawEventResult {
	...
	if result, ok := parseWholeBufferOpenclawResult(buf, b.cfg.GatewayMode); ok {
...
func parseWholeBufferOpenclawResult(buf []byte, gatewayMode bool) (openclawResult, bool) {
	trimmed := strings.TrimSpace(string(buf))
	if trimmed == "" {
		return openclawResult{}, false
	}
	if result, ok := tryParseOpenclawResult(trimmed, gatewayMode); ok {
		return result, true
	}
	...
	for i, line := range lines {
		if len(line) > 0 && line[0] == '{' {
			...
			if result, ok := tryParseOpenclawResult(candidate, gatewayMode); ok {
				return result, true
			}
			return openclawResult{}, false
		}
