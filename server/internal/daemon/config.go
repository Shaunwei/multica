...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if e, ok := probe("MULTICA_OPENCLAW_PATH", "openclaw", "MULTICA_OPENCLAW_MODEL"); ok {
		...
		openclawAgentName := strings.TrimSpace(os.Getenv("MULTICA_OPENCLAW_AGENT_NAME"))
		e.GatewayMode = openclawGateway
		e.SessionPrefix = openclawSessionPrefix
		e.AgentName = openclawAgentName
		agents["openclaw"] = e
	}
